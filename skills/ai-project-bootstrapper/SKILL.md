...
| `DURABLE` | **默认档位**。适用于所有需要长期 AI 维护的项目；也适用于多人或多 AI 协作、多个稳定工作流/入口，或高安全/硬件/财务/数据一致性要求 | 必须建立 `AGENTS.md`、`ai-context/INDEX.md`（目录→职责映射表）、根任务路由；按需添加架构主题和操作记录；不创建空 Bug 记录或推测性主题 |
